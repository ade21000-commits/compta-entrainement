...
st.set_page_config(page_title="BODEGA - Saisie table", page_icon="🍷", layout="wide")
...
CODES = list(PLAN_COMPTABLE.keys())
...
def clean_rows(df: pd.DataFrame) -> pd.DataFrame:
    """Garde les lignes non vides et normalise les types."""
    ...
    df = df[util].copy()
    # Recalcule systématiquement l'intitulé à partir du code
...
# -------- En-tête opération --------
st.title("🍷 Saisie par tableau (avec libellés)")
...
st.caption("Saisis chaque ligne : choisis un **code** (colonne *Compte*), le **libellé** se remplit tout seul.")

# -------- Tableau éditable : code + libellé auto --------
edited = st.data_editor(
    ...
    hide_index=True,
    column_config={
        # Sélecteur sur les CODES, le libellé apparaît à côté dans la colonne suivante
        "Compte": st.column_config.SelectboxColumn(
            "Compte",
            options=[""] + CODES,
            help="Choisis un code (ex: 607, 512...)"
        ),
        # Colonne d'affichage des libellés (on la recalcule après édition)
        "Intitulé compte": st.column_config.TextColumn(
            "Intitulé compte",
            help="Rempli automatiquement à partir du code",
            disabled=True,          # lecture seule (si version de Streamlit le permet)
            width="large"
        ),
...
# Recalcule des libellés et nettoyage
op_df = clean_rows(edited)

# Mémorise l'état nettoyé pour la prochaine édition (garde la colonne libellé à jour)
# + ajoute une ligne vide si tout est rempli pour encourager la saisie continue
if len(op_df) == 0 or not (op_df.iloc[-1][["Compte","Libellé ligne","Débit","Crédit"]] == ["", "", 0.0, 0.0]).all():
    # Ajoute une ligne vide en plus
    st.session_state.grid = pd.concat(
        [op_df, pd.DataFrame([{"Compte":"", "Intitulé compte":"", "Libellé ligne":"", "Débit":0.0, "Crédit":0.0}])],
        ignore_index=True
    )
else:
    st.session_state.grid = op_df.copy()
...
cA.metric("Total Débit", euro(total_d))
cB.metric("Total Crédit", euro(total_c))
with cC:
...
# -------- Vérifs par ligne (simples et utiles) --------
errors = []
for i, row in op_df.reset_index(drop=True).iterrows():
    code = str(row["Compte"]).strip()
    libligne = str(row["Libellé ligne"]).strip()
    d, c = float(row["Débit"]), float(row["Crédit"])
    if code == "":
    ...
    if (d > 0 and c > 0):
        errors.append(f"Ligne {i+1} : débit **et** crédit saisis (choisir un seul sens).")
    ...
    if st.button("✅ Valider l'opération", type="primary", use_container_width=True, disabled=disabled):
        ...
        for _, r in op_df.iterrows():
            st.session_state.journal.append({
                ...
                "Compte": r["Compte"],
                "Intitulé compte": PLAN_COMPTABLE.get(r["Compte"], ""),
                "Libellé ligne": r["Libellé ligne"],
                "Débit": r["Débit"],
                "Crédit": r["Crédit"]
            })
        # Reset table + incrément pièce
        st.session_state.grid = pd.DataFrame([{
            "Compte":"", "Intitulé compte":"", "Libellé ligne":"", "Débit":0.0, "Crédit":0.0
        }])
...
if len(st.session_state.journal) == 0:
    ...
    a,b,c = st.columns(3)
    a.metric("Écritures", len(J)); b.metric("Total Débit", euro(td)); c.metric("Total Crédit", euro(tc))

    # Export optionnel (Excel)
    out = BytesIO()
